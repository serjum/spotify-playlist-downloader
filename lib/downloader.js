...
(function() {
  var Downloader, Error, EventEmitter, ListProperties, Log, Path, Playlist, SpotifyWeb, Success, Track, async, colors, domain, fs, id3, lodash, mkdirp, replaceAll,
  ...
  replaceAll = (function(_this) {
    return function(search, replacement, target) {
  ...
  Downloader = (function(superClass) {
    ...
    function Downloader(username, password) {
      ...
      this.loadTrack = bind(this.loadTrack, this);
    ...
    Downloader.prototype.loadSpotifyItem = function(callback) {
      if (this.playlist.indexOf('https://play.spotify.com') !== -1) {
        this.playlist = this.playlist.replace('https://play.spotify.com', 'spotify');
        this.playlist = replaceAll('/', ':', this.playlist);
      }
      Log(this.playlist);
      if (this.playlist.indexOf('album') !== -1) {
        return this.loadAlbum(callback);
      } else if (this.playlist.indexOf('track') !== -1) {
        return this.loadTrack(callback);
      } else {
        return this.loadPlaylist(callback);
      }
    };

    Downloader.prototype.loadTrack = function(callback) {
      Log('Getting Track Data');
      this.Playlist.setEnabled(false);
      this.trackUrls = [this.playlist];
      return typeof callback === "function" ? callback() : void 0;
    };
